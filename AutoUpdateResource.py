...
def calculate_git_sha1(filepath):
    ...
    try:
        ...
        log(f"计算 {filepath} 哈希失败: {str(e)}", 'error')
        return None

def generate_manifest():
    """生成初始清单文件"""
    if not LOCAL_RESOURCE.exists():
        log(f"资源目录 {LOCAL_RESOURCE} 不存在", 'error')
        return False
    ...
    try:
        ...
        log(f"清单文件已生成 | 路径: {MANIFEST_FILE}")
        log(f"统计: 记录了 {file_count} 个文件")
        return True
    except IOError as e:
        log(f"写入清单文件失败: {str(e)}", 'error')
        return False

def load_local_manifest():
    """加载本地SHA记录"""
    if os.path.exists(MANIFEST_FILE) == False:
        log("未找到清单文件，开始生成")
        generate_manifest()
